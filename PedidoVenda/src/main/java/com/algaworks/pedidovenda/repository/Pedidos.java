...
public class Pedidos implements Serializable {
	...
	public Map<String, BigDecimal> valorUsuario() {
		...
		criteria.setProjection(Projections.projectionList()
				.add(Projections.groupProperty("vendedor").as("usuario"))
				.add(Projections.sum("valorTotal").as("valor")));
		
		List<UsuarioValor> valores = criteria
				.setResultTransformer(Transformers.aliasToBean(UsuarioValor.class)).list();
		
		for (UsuarioValor usuario : valores) {
			resultado.put(usuario.getUsuario().getNome(), usuario.getValor());
		}
